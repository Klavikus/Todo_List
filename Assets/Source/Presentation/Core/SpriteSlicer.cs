...
public static class SpriteSlicer2D
{
    ...
    {
        get { return s_DebugLoggingEnabled; }
        set { s_DebugLoggingEnabled = value; }
    }
        ...
        int maxCutDepth, ref List<SpriteSlicer2DSliceInfo> slicedObjectInfo)
    {
        ...
        SliceSpritesInternal(worldStartPoint, worldEndPoint, null, 0, destroySlicedObjects, maxCutDepth,
            ref slicedObjectInfo, layerMask);
    }
    ...
    public static void SliceSprite(Vector3 worldStartPoint, Vector3 worldEndPoint, GameObject sprite)
    {
        LayerMask layerMask = -1;
        List<SpriteSlicer2DSliceInfo> slicedObjectInfo = null;
        SliceSpritesInternal(worldStartPoint, worldEndPoint, sprite, 0, true, -1, ref slicedObjectInfo, layerMask);
    }
    ...
    static void SliceSpritesInternal(Vector3 worldStartPoint, Vector3 worldEndPoint, GameObject spriteObject,
        ...
        if (cutStartResults.Length == cutEndResults.Length)
        {
                 ...
                 cutResultIndex++)
            {
                ...
                if (cutEnter.rigidbody == cutExit.rigidbody)
                {
...
#endif
                    {
                        ...
                        if (parentUnitySprite == null)
                        {
                            ...
                            if (parentSlicedSprite == null || (maxCutDepth >= 0 &&
                                                               parentSlicedSprite.CutsSinceParentObject >= maxCutDepth))
                            {
                                continue;
                            }
                        }
                    ...
                    Vector3 cutStartLocalPoint =
                        parentRigidBody.gameObject.transform.InverseTransformPoint(worldStartPoint);
                    Vector3 cutEndLocalPoint =
                        parentRigidBody.gameObject.transform.InverseTransformPoint(worldEndPoint);

                    Vector3 cutEnterLocalPoint =
                        parentRigidBody.gameObject.transform.InverseTransformPoint(cutEnter.point);
                    Vector3 cutExitLocalPoint =
                        parentRigidBody.gameObject.transform.InverseTransformPoint(cutExit.point);
                        ...
                        else
                        {
                            ...
                            if (circleCollider)
                            {
                                ...
                                for (int loop = 0; loop < numSteps; loop++)
                                {
                                    ...
                                    polygonPoints[loop] = new Vector2(Mathf.Sin(angle), Mathf.Cos(angle)) *
                    ...
                    if (polygonPoints != null)
                    {
                        ...
                        if (!AreVerticesAcceptable(childSprite1Vertices) ||
                            !AreVerticesAcceptable(childSprite2Vertices))
                        {
                            continue;
                        }
                        else
                        {
                            ...
                            CreateChildSprite(parentRigidBody, physicsMaterial, childSprite1Vertices, parentArea,
                                out childSprite1, out child1Collider);
                            childSprite1.gameObject.name = parentRigidBody.gameObject.name + "_child1";

                            CreateChildSprite(parentRigidBody, physicsMaterial, childSprite2Vertices, parentArea,
                                out childSprite2, out child2Collider);
    ...
    /// </summary>
    static void CreateChildSprite(Rigidbody2D parentRigidBody, PhysicsMaterial2D physicsMaterial,
        List<Vector2> spriteVertices, float parentArea, out SlicedSprite slicedSprite,
    ...
    static bool IsPointInsidePolygon(Vector2 pos, Vector2[] polygonPoints)
    {
        ...
        for (int vertexIndex = 0; vertexIndex < polygonPoints.Length; vertexIndex++)
        {
            ...
            if (thisPoint.y <= pos.y)
            {
                ...
                {
                    float isLeft = ((nextPoint.x - thisPoint.x) * (pos.y - thisPoint.y) -
        ...
        for (int loop = 0; loop < Selection.gameObjects.Length; loop++)
        {
            ...
            if (polyCollider)
            {
                ...
                if (SpriteSlicer2D.IsConvex(vertices))
                {
                    ...
                    Debug.Log(Selection.gameObjects[loop].name + " points reduced to " + vertices.Count.ToString() +
                              " from " + originalNumVertices.ToString());
                }
                else
                {
...
public class SlicedSprite : MonoBehaviour
{
    ...
    {
        get { return m_CutsSinceParentObject; }
    }
    ...
    {
        get { return m_HFlipped; }
    }

    public bool VFlipped
    {
        get { return m_VFlipped; }
    }
    ...
    {
        InitSprite(slicedSprite.gameObject, polygon, slicedSprite.MinCoords, slicedSprite.MaxCoords,
            slicedSprite.SpriteBounds, slicedSprite.m_MeshRenderer.sharedMaterial, slicedSprite.Rotated,
            slicedSprite.HFlipped, slicedSprite.VFlipped);
    ...
    public void InitFromUnitySprite(SpriteRenderer unitySprite, PolygonCollider2D polygon)
    {
        ...
        Vector2 minTextureCoords = new Vector2(textureRect.xMin / (float) unitySprite.sprite.texture.width,
            textureRect.yMin / (float) unitySprite.sprite.texture.height);
        Vector2 maxTextureCoords = new Vector2(textureRect.xMax / (float) unitySprite.sprite.texture.width,
            textureRect.yMax / (float) unitySprite.sprite.texture.height);

        InitSprite(unitySprite.gameObject, polygon, minTextureCoords, maxTextureCoords, unitySprite.sprite.bounds,
    ...
    /// </summary>
    void InitSprite(GameObject parentObject, PolygonCollider2D polygon, Vector3 minCoords, Vector3 maxCoords,
        Bounds spriteBounds, Material material, bool rotated, bool hFlipped, bool vFlipped)
    {
