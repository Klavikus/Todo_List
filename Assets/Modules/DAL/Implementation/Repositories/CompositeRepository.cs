...
{
    public class CompositeRepository : IProgressRepository
    {
        ...
        {
            foreach (Type type in progressRepository.HandledTypes)
            {
                _reposByTypes[type].Clear();

                foreach (IEntity entity in progressRepository.GetAll(type))
